...
Обновлено: 2026-09-16. **PR-02–PR-07 приняты; PR-08 подготовлен локально** в `agent/feat/delivery-runtime-gate` от `main` (`befb159`). PR-04/PR-05 приняты в приложении: текущий deployment и evidence проверены через GitHub App. [Контракт PR-08](delivery_runtime.md) описывает допуск, остановку, бюджеты, наблюдение и оставшиеся границы transport. Push и merge выполняет владелица. PR-01 отложен; PR-09–PR-15 и пилот ещё предстоят. Идентификаторы этапов не совпадают с номерами GitHub PR; исторические записи ниже сохраняют состояние на свою дату.
...
| --- | --- | --- |
| Symphony | `D:/symphony`, PR-08 в `agent/feat/delivery-runtime-gate` от `main` (`befb159`) | Все дальнейшие изменения Symphony выполнять здесь; PR направлять в личный fork с базой `main` |
