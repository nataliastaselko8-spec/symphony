...
**Решение владелицы от 2026-09-18 — после первого пилота:** распространение
версионированного установочного комплекта и его автоматическое получение через
`Symphony.cmd Setup` запланированы отдельным этапом после первого пилотного запуска
и разбора его результатов. Сейчас реализацию и публикацию не начинать. Для первого
пилота используется подготовленный локальный комплект; `.runtime-local/` исключена
из Git и не появляется у другого разработчика после clone. Общие компоненты fork
и закрытый профиль EmotionStat должны распространяться с раздельным учётом доступа;
личные настройки и секреты остаются вне поставки. Подробный объём и приёмка —
[раздел 8 плана установщика](../../tools/wsl/installer-plan.md). Это не блокер PR-14
и не изменение нумерации PR-14/PR-15; отдельный PR уточняется по итогам пилота.
...
**Уточнение переносимости от 2026-09-17:** общий runtime, launcher/preflight, setup и
шаблоны поставляются в публичном fork Symphony (PR-11). `EmotionStat/agent-runner`
содержит проектные правила и шаблон WORKFLOW (PR-12). Имена WSL, accounts, host UID/GID,
пути и порты каждого разработчика находятся в его локальном config вне git.
Пути/версии машины владелицы в истории настройки ниже не становятся обязательными defaults.
Подтверждён режим: один назначенный активный runner на проект; остальные установки —
для настройки и чтения. Локальный flock не считается распределённой защитой между компьютерами.
...
| O2 — controller/worker | **PR-11: техническая граница проверена; live bootstrap ещё не завершён** | Rootless Podman, управляющий и task SSH, файловые ограничения, scoped IPv4/pasta и IPv6 canary, остановка/экспорт и восстановление проверены. Та же программа прошла под временным другим UID. См. [отчёт PR-11](github_projects_setup/pr11-validation.md). Отдельный Codex login, рабочий профиль и подключение к startup — PR-12/PR-13; задачи не запущены |
...
| O5 — GitHub rules и граница публикации | Проверить реальные возможности тарифа и действующие rules; для выбранного профиля без доступной branch protection доказать ограничения controller publisher и отсутствие GitHub credentials у worker | После PR-04/PR-09/PR-11; доступные серверные rules использовать без bypass. Недоступные rules не объявлять включёнными и не требовать покупки тарифа для PR-11. Broker ограничивает агента, но не владельца и другие credentials; review/merge остаются ручными. Живую `dev` не тестировать пробным запрещённым push |
...
**Подробный план для валидации:** [порядок реализации PR-08](github_projects_setup/pr08-execution-plan.md), включая единственный допуск запуска, бюджеты, отмену, подтверждение остановки, cleanup, hooks, restart/reload и сценарии приёмки.

**Repo/base:** `nataliastaselko8-spec/symphony` → `main`. **Title:** `Gate agent dispatch and preserve delivery ownership across retries`.

**Результат PR-08:** [реализация и эксплуатационные границы](delivery_runtime.md).
Сохранённый цикл управляет допуском и cleanup, одноразовым разрешением PID, отменой,
монотонным временем и отдельным observer. Startup Projects остаётся закрытым.
Транспорт с внешними эффектами по умолчанию остаётся `stop_unconfirmed` до доказанной
остановки; реальная реализация/приёмка SSH/Podman относится к PR-11. Scope старых store
изменился: автоматической миграции нет. Панель и публикация остаются PR-10 и PR-09.
...
**Подробный план для валидации:** [шаги реализации PR-09](github_projects_setup/pr09-execution-plan.md). Уточняет раннее сохранение PR для CI, остановку worker перед публикацией, восстановление внешних операций и влияние событий `edited`/`ready_for_review` на бюджет. Новые варианты поведения в документе пока предложены, реализация не начата.
...
### PR-11 — Подготовить переносимый runtime и launcher

**Repo/base:** `nataliastaselko8-spec/symphony` → `main`. **Title:** `Add a portable isolated runtime with local configuration and read-only preflight`.

**Подробный план для валидации:** [PR-11 — порядок реализации и приёмка](github_projects_setup/pr11-execution-plan.md).
Он уточняет границу: PR-11 реализует и отдельно проверяет worker transport, stop proof и Git export;
подключение этих компонентов к обычному startup движка и разрешение исполнения остаются в PR-13.
Текущий `--dry-run` — конечная инспекция без dashboard. Отсутствие платных GitHub rules
не блокирует разработку runtime: возможности сервера фиксируются отдельно от ограничений controller publisher.

**Цель / было → станет:** разработчик скачивает fork и настраивает его без редактирования исходников под свою WSL/account/файлы; default действие остаётся read-only. Один назначенный controller исполняет задачи, остальные установки — настройка/inspection.

**Объём и файлы:** общие скрипты находятся в fork: `runtime/scripts/configure.sh`, `launch.sh`, `preflight.sh`, `status.sh`, `stop.sh`; image/host templates и схемы — `runtime/`. Обновляются README, `elixir/README.md`, ignore и параметризуется существующий демо-launcher с личными путями. Первый проверяемый runtime — Linux в WSL2 с rootless Podman; Windows wrapper принимает выбранные distro/account из config. Имена distro, host UID/GID, диски, checkout и порты не зашиты. Поддержка native Windows/macOS/отдельного сервера не объявляется без отдельной проверки. Проектный профиль остаётся в agent-runner PR-12.

- Общий runtime lock закрепляет образ/инструменты и контракты; локальный deployment manifest — точные Symphony commit/digest, ревизию проектного профиля и версию config. Собственный будущий merge SHA не записывается в этот же PR, плавающий `main` не подменяет pin. PR-13 проверяет совместимость, O7 заполняет итоговые значения после merge серии. Repo-owned изменения pin также проходят через PR; начальная сборка допускает только инспекцию/тесты.
...
- **Обязательное требование владелицы:** процессы задачи не читают, не просматривают и не изменяют файлы/папки хоста вне выделенных ресурсов задачи. Доступны только её workspace, необходимые scratch/cache, собственная авторизация Codex и инструменты контейнерного image. Host root/home/system dirs нельзя подключить даже read-only; local config не разрешает произвольные mounts или отключение изоляции. Проверить настоящие SSH/hooks/child processes, обходы путей и restart на безопасных host-маркерах. Неуспех закрывает исполнение. Полный файловый контракт — в плане PR-11.
- Профиль rootless Podman запускает SSH и worker внутри контейнера с отдельными mount/PID/IPC/network namespaces. На текущей машине проверяется подготовленный `Ubuntu-26.04`; другие установки выбирают свой совместимый distro. Не использовать host networking, mounts controller home, общих WSL/WSLg путей или Docker/Podman management sockets. SSH публикуется на loopback-портах из local config; 2223/2224 — предложения для машины владелицы. Проверить реальные host endpoints, включая исходящие соединения. Не ослаблять AppArmor/user namespaces глобально; совместимость SSH/PAM проверить в выбранном образе. Профиль ещё не прошёл приёмку.
...
- `runtime/scripts/launch.sh` определяет свою поставку от расположения скрипта; проектный шаблон получает из явного local config. Configure формирует исполняемый WORKFLOW из профиля и объявленных машинных параметров, без `eval`, credentials в тексте и зависимости от cwd. Скрипты принадлежат fork; проектный шаблон/hooks — agent-runner; полученные local config/WORKFLOW/manifest вне git. При нескольких WSL distro выбор явный; пути с пробелами/Unicode и другое имя account проверяются тестами.
...
**Проверки/evidence:** установка из нового clone с другим account/UID/distro/путями/портами без правки исходников; отсутствие личных literals в общих executable/templates; второй локальный запуск; неверная версия/путь; отсутствующий bash/Codex/git; ошибочный SSH host; worker не читает controller secrets/state и не получает GitHub credentials; publisher соблюдает scope/branch restrictions; отсутствие shared controller home и SSH agent forwarding; read-only default без side effects. Проверки локального lock не выдаются за защиту нескольких независимых controller.
...
**Repo/base:** `EmotionStat/app` → `dev`. **Выбранная задача:** [№132 — Изменить цвет кнопки «Поделиться» на серый](https://github.com/EmotionStat/app/issues/132). **PR:** [№133 — fix(web): make share question button gray](https://github.com/EmotionStat/app/pull/133).

**Дополнение подготовки от 2026-09-18:** в Symphony запланирован
[учёт физического диска Windows](../../tools/wsl/installer-plan.md#9-контроль-свободного-места-на-физическом-диске-windows):
общий остаток тома для WSL-сред, свежие измерения manager, пороги предупреждения
и остановки, корректный dashboard и обновление без потери настройки/работы.
Это отдельная доработка инфраструктуры, не scope продуктовой issue PR-14;
она пока не реализована. Нынешнее `ready` учитывает свободное место внутри WSL
и не подтверждает достаточный остаток физического Windows-диска.

**Результат запуска на 2026-09-18:** PR №133 прошёл CI на candidate
`a213d64436820e5adab71fd483a3fb172f0e77ad`; controller подтвердил native linkage
и перевёл карточку в `PR ready`. Запуск потребовал исправлений runtime и отдельного
повтора отклонённого запроса создания PR, поэтому проход без технического вмешательства
ещё не подтверждён. Последующая сверка: PR слит в `dev` с SHA
`b3b7c17a6b12dc3afe62e2a5be58cd6bc645b947`,
[deployment 35366428380, attempt 1](https://github.com/EmotionStat/app/actions/runs/35366428380)
успешен; положительная ручная проверка оператора сохранена для того же SHA/run/attempt.
Внутренний цикл `completed`, однако карточка всё ещё `PR ready`: автоматическая
передача этапов после merge в Project Status пока не реализована и добавлена в план.
[План исправлений первого пилота](../../tools/wsl/installer-plan.md#11-исправления-по-результатам-первого-пилота)
фиксирует шесть применённых локально исправлений, согласованную политику offline/CI,
выпуск нового комплекта, обновление без потери данных и оставшуюся приёмку. Дополнение
по запросу владелицы охватывает своевременную синхронизацию карточки на каждом этапе,
включая аварии до PR: таблица переходов, целевые сроки, сохранённые операции записи,
видимая задержка при недоступном GitHub и продолжение после решения оператора без
ручного перетаскивания карточки. Эта общая синхронизация ещё не реализована.
Изменения ещё не закоммичены и не включены в новый комплект; готовность текущей
установки не означает готовности выпуска для остальных разработчиков.
...
**Зависимости:** PR-13, completed O2–O8, live permissions/rules/schema проверены. Scope выбранной issue №132 — три значения `background` в `apps/web/src/features/share-question/ui/ShareQuestion.module.css`; исправления Symphony не входят в продуктовый PR.
