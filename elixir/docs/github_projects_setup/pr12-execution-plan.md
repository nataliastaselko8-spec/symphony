...
Дата: 2026-09-17. Статус: **PR12 реализован и прошёл локальную приёмку; готов к review**.
После согласованного предела 256 MiB прошли все 36 локальных шагов приложения,
kernel canary, проверки изоляции и отмены. Push/merge и hosted CI ещё не выполнялись.
...
Владелица явно согласовала повышение до 256 MiB на файл. В runtime меняется только
RLIMIT_FSIZE; 2 GiB/2 CPU, bundle 80 MiB, filesystem/network isolation сохраняются.
Повторная приёмка проверяет успешную запись 151 356 536 bytes и отказ выше 256 MiB.
...
Companion revision `1f6944c` включает полную запись Git-bundle фреймов и согласованный
предел файла 256 MiB. Runner закрепляет эту ревизию; перед его hosted CI
