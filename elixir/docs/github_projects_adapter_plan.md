...
Обновлено: 2026-09-17. PR-02–PR-09 приняты; PR-10 подготовлен локально в `D:/symphony`, ветка `agent/feat/operator-dashboard`. Реализованы внутренний runtime и [защищённая панель с тестовым демо](operator_dashboard.md); реальное выполнение Projects остаётся отключено. Владелица выбрала ручной источник: отдельное подтверждение Queue/Scheduler с привязкой к deployment/artifact/policy и последующая validation dev в течение 30 минут. Дополнение входит в тот же PR-10. Исходный [макет](github_projects_ui/recovery-interface-preview.html) сохранён для истории; актуальные границы и остатки — в [rollout](github_projects_pr_rollout_plan.md).
