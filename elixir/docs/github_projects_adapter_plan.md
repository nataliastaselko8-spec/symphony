...
**План PR13 подготовлен для валидации:** [интеграция runtime, launcher, dashboard и системная приёмка](github_projects_setup/pr13-execution-plan.md). Этап включает кодовое подключение prepare/start/heartbeat/stop/export, единый cwd, защищённый startup и устойчивый лимит одного пилота; минимальный companion профиля в agent-runner нужен для совместимости. Реализация PR13 не начата, реальная продуктовая задача остаётся PR14.
