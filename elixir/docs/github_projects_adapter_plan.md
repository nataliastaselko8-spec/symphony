...
**Решение владелицы зафиксировано; реализация ещё предстоит.** Это отдельные бюджеты controller для `github_projects`, а не уже существующие параметры WORKFLOW или переименование `max_turns`/транспортных retries. Значения ниже действуют совместно: достижение любого применимого лимита прекращает автоматические попытки, сохраняет работу и удерживает цикл репозитория.
