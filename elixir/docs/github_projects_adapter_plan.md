...
**Уточнение границ PR-12:** проектный шаблон/hooks и производный image находятся в `agent-runner`, исполняемый WORKFLOW — в локальном config вне git, общий launcher — в Symphony. Controller доставляет проверенный dev bundle и публикует результат через PR-09; worker не делает сетевые fetch/push и не получает GitHub credentials. Docker build app остаётся обязательным в CI; локальный отчёт отмечает его `CI_ONLY`. [План PR-12](github_projects_setup/pr12-execution-plan.md) уточняет прежние формулировки о git hooks и перечисляет оставшиеся точки интеграции PR-13.
