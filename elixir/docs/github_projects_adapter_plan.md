...
Дата: 2026-09-15. Статус: PR-02 слит; PR-03 реализовал controller credentials и опубликован как draft PR #2; runtime исполнения/подтверждения и backend панели ещё не реализованы. Интерактивный интерфейс на текущем этапе — [макет](github_projects_ui/recovery-interface-preview.html); [desktop](github_projects_ui/manual-validation-desktop.png) и [mobile](github_projects_ui/manual-validation-mobile.png) изображения сохранены рядом.
...
В `runner.example.yaml` уже есть `dry_run: true`, но Symphony этот файл не читает. Для MVP не создавать второй формат настроек движка: `D:/agent-runner/scripts/launch.sh` передаёт Symphony абсолютный WSL-путь к `D:/agent-runner/WORKFLOW.md` и аргументы CLI. `D:/agent-runner/scripts/preflight.sh` выполняет предварительную проверку; скрипты запуска и рабочий профиль создаются в PR-11/PR-12 репозитория `EmotionStat/agent-runner`. Launcher определяет профиль относительно своего расположения, а не случайного cwd терминала; Linux-пути проверяются при настройке WSL2. Старый пример обновить при настройке runner, явно указав, что является параметром launcher, а что — Symphony.

Владелец подтвердил runtime: **WSL2 на этом компьютере**. Controller и отдельный worker настраиваются в Linux внутри WSL2; наличие дистрибутива, зависимости и изоляция ещё должны пройти preflight. Проверенная сборка Symphony содержит Linux/macOS targets; hooks требуют `sh`, запуск Codex — `bash`. Для исполнения задаётся единая система Linux-путей: Windows-папка исходников `D:/fork/_symphony/symphony` и `D:/agent-runner` требуют явного сопоставления с путями внутри WSL2. Нативный Windows не входит в выбранный MVP. Строки PowerShell нельзя напрямую вставлять в `sh -lc` hooks.
...
**Решение владелицы зафиксировано; в PR-06 реализованы хранение, счётчики и резервирование.** [Контракт PR-06](delivery_cycle.md) пока не подключён к runtime: измерение/остановка worker, реальные повторы Actions и панель оператора относятся к PR-08/09/10. Это отдельные бюджеты controller для `github_projects`, а не переименование `max_turns`/транспортных retries. В исполняемом профиле значения ниже действуют совместно: достижение любого применимого лимита прекращает автоматические попытки, сохраняет работу и удерживает цикл репозитория.
